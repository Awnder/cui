...
def sequence_sorted_lists(sequences):
    """ Custom: Sorts the sequence dictionary in ascending order. Input and outputs are a dictionaries"""
    sorted_sequence = dict(sorted(sequences.items(), key=lambda item: len(item[1]), reverse=True))
...
def find_common_factors(factors):
    """ Finds the common factors within multiple groups of factors. Input is a
        ...
        For input { "ABC": [2,4], "BCD": [4], "EFG": [] }, it returns []
    """
    commonFactors = []
...
def find_closest_factors(factors):
    """ Finds common factors within multiple groups of factors. Input is a
        dictionary of sequences and all the factors of their span length.
        Returns a dictionary of factors and the count of appearance that appear most frequently.
        For input { "ABC": [2,4], "BCD": [4], "EFG": [] }, it returns [4]
    """
    commonFactors = {}
    for value_list in factors.values():
        for v in value_list:
            if v in commonFactors:
                commonFactors[v] += 1
            else:
                commonFactors[v] = 1

    sortedCommonFactors = dict(sorted(commonFactors.items(), key=lambda item: item[1], reverse=True))    
    return sortedCommonFactors
...
def topN(subtext, n):
    """ Does the same thing as top3 but your choice of popping """
    q = _queue.PriorityQueue()
    scores = _caesar.score_all_keys(subtext)
    for i, score in scores.items():
        keyLetter = chr(i + ord('A'))
